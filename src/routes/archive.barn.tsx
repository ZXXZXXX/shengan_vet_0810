          ...
          {barns.map((b) => (
            <div key={b.id} className="flex items-center gap-4 px-6 h-14 text-table-cell border-b border-border last:border-0 hover:bg-surface-subtle">
              <div className="grid grid-cols-6 gap-4 flex-1 min-w-0">
                ...
                <div className="text-body-sm text-text-secondary truncate">{b.farm}</div>
                <div className="text-body-sm text-text-secondary truncate" title={b.desc}>{b.desc}</div>
                <div className="truncate"><span className="tag tag-muted">{b.type}</span></div>
                <div className="tabular-nums text-body text-foreground truncate">{b.stock} <span className="text-caption text-text-tertiary">头</span></div>
              </div>
