...
const diseaseKB_disease: DiseaseEntry[] = [
  {
    ...
    plans: [
      { id: "p1", rx: "处方 1 · 头孢噻呋 · 肌注 + 氟尼辛葡甲胺 · 静推", desc: "疗程 3-5 天，配合外阴冲洗与碘甘油局部处理", drugs: ["5% 盐酸头孢噻呋（畜可健） 4.4mL/100kg 肌注", "氟尼辛葡甲胺（福欣安） 4mL/100kg 静推"], duration: "3-5 天" },
      { id: "p2", rx: "处方 2 · 头孢噻呋 · 肌注 + 氟尼辛葡甲胺 · 静推", desc: "3 天 1 次给药；损伤 >5cm 须 PGA 缝合", drugs: ["10% 盐酸头孢噻呋 20mL/次 3 天 1 次", "氟尼辛葡甲胺（福欣安） 4mL/100kg 静推"], duration: "3-5 天" },
    ],
  },
  {
    name: "产后子宫炎",
    ...
    plans: [
      { id: "p1", rx: "处方 1 · 青霉素钠 · 肌注 + 氟尼辛葡甲胺 · 静推", desc: "产后 10 天内，早晚各 1 次", drugs: ["注射用青霉素钠（联治灵） 2.2 万 IU/kg 肌注", "氟尼辛葡甲胺（福欣安） 4mL/100kg 静推"], duration: "3 天" },
      { id: "p2", rx: "处方 2 · 头孢噻呋 · 肌注 + 氟尼辛葡甲胺 · 静推", desc: "产后 10 天内，1 天 1 次", drugs: ["5% 盐酸头孢噻呋（畜可健） 4.4mL/100kg 肌注", "氟尼辛葡甲胺（福欣安） 4mL/100kg 静推"], duration: "3 天" },
      { id: "p3", rx: "处方 3 · 头孢噻呋 · 肌注 + 氟尼辛葡甲胺 · 静推 + 利福昔明 · 子宫灌注", desc: "产后 5 天以上；利福昔明子宫灌注 2-3 次", drugs: ["10% 盐酸头孢噻呋 20mL 3 天 1 次", "氟尼辛葡甲胺（福欣安） 4mL/100kg 静推", "利福昔明子宫注入剂（澳利舒） 100mL/次"], duration: "3 天" },
    ],
  },
  {
    name: "子宫内膜炎",
    ...
    plans: [
      { id: "p1", rx: "处方 1 · 青霉素钠 · 肌注 + 氟尼辛葡甲胺 · 静推", desc: "直肠按压排脓后用药，早晚各 1 次", drugs: ["注射用青霉素钠（联治灵） 2.2 万 IU/kg 肌注", "氟尼辛葡甲胺（福欣安） 4mL/100kg 静推"], duration: "3 天" },
      { id: "p2", rx: "处方 2 · 利福昔明 · 子宫灌注", desc: "利福昔明 100 mL/次，2 天一次", drugs: ["利福昔明子宫注入剂（澳利舒） 100mL/次"], duration: "2-3 次" },
    ],
  },
];
...
const diseaseKB_hoof: DiseaseEntry[] = [
  {
    ...
    plans: [
      {
        id: "p1",
        rx: "处方 1 · 修长度 · 护理 + 修平整度 · 护理 + 修角度 · 护理 + 清理趾间与特殊结构 · 护理 + 检查与修复 · 护理",
...
const diseaseKB_drying: DiseaseEntry[] = [
  {
    ...
    plans: [
      { id: "p1", rx: "处方 1 · 硫酸头孢喹肟（牧全欣）· 乳注", desc: "3 g/支，按非盲乳数一次量乳注", drugs: ["硫酸头孢喹肟乳房注入剂 干乳期（牧全欣） 3 g/支"], duration: "1 次处理" },
      { id: "p2", rx: "处方 2 · 硫酸头孢喹肟（茹通）· 乳注", desc: "3 g/支，按非盲乳数一次量乳注", drugs: ["硫酸头孢喹肟乳房注入剂（干乳期）（茹通） 3 g/支"], duration: "1 次处理" },
      { id: "p3", rx: "处方 3 · 硫酸头孢喹肟（海喹宁）· 乳注", desc: "3 g/支，按非盲乳数一次量乳注", drugs: ["硫酸头孢喹肟乳房注入剂（干乳期）（海喹宁） 3 g/支"], duration: "1 次处理" },
      { id: "p4", rx: "处方 4 · 盐酸头孢噻呋（畜可健）· 乳注", desc: "8 ml/支，按非盲乳数一次量乳注", drugs: ["盐酸头孢噻呋乳房注入剂 干乳期（畜可健） 8 ml/支"], duration: "1 次处理" },
      { id: "p5", rx: "处方 5 · 硫酸头孢喹肟（赛福魁）· 乳注", desc: "3 g/支，按非盲乳数一次量乳注", drugs: ["硫酸头孢喹肟乳房注入剂 干乳期（赛福魁） 3 g/支"], duration: "1 次处理" },
    ],
  },
];
...
const diseaseKB_postpartum: DiseaseEntry[] = [
  {
    ...
    plans: [
      {
        id: "p1",
        rx: "处方 1 · 盐酸头孢噻呋 · 肌肉注射/皮下注射 + 氟尼辛葡甲胺 · 静脉注射 + 直肠体温 · 检查 + 情况评估 · 检查",
        desc: "难产助产 3/4 分、双胎、死胎等牛只预防生殖系统感染；5% 盐酸头孢噻呋 1 天 1 次连用 3 天，氟尼辛葡甲胺 1 天 1 次连用 3 天；同步 14 天产后检查。产后 7 天体温监测异常时进一步体格检查并按对应疾病方案治疗",
        drugs: [
          "5% 盐酸头孢噻呋注射液（畜可健） · 肌肉注射/皮下注射 · 1 天 1 次，连用 3 天 · 体重区间剂量：200-400kg → 10ml/次；400-600kg → 20ml/次；600-900kg → 30ml/次；900kg 及以上 → 35ml/次",
          "氟尼辛葡甲胺注射液（福欣安） · 静脉注射 · 1 天 1 次，连用 3 天 · 体重区间剂量：200-400kg → 10ml/次；400-600kg → 20ml/次；600-900kg → 30ml/次；900kg 及以上 → 35ml/次",
          "直肠体温 · 检查 · 测量并记录牛只直肠体温 · 数字输入 · 14 天，1 天 1 次",
          "情况评估 · 检查 · 拍摄牛只正前方、正后方照片 · 图片视频 · 14 天，1 天 1 次",
        ],
        duration: "14 天",
      },
      {
        id: "p2",
        rx: "处方 2 · 盐酸头孢噻呋 · 肌肉注射/皮下注射 + 氟尼辛葡甲胺 · 静脉注射 + 直肠体温 · 检查 + 情况评估 · 检查",
        desc: "难产助产 3/4 分、双胎、死胎等牛只预防生殖系统感染；10% 盐酸头孢噻呋 3 天 1 次，氟尼辛葡甲胺 1 天 1 次连用 3 天；同步 14 天产后检查。产后 7 天体温监测异常时进一步体格检查并按对应疾病方案治疗",
        drugs: [
          "10% 盐酸头孢噻呋注射液（畜可健 / 欣利达） · 肌肉注射/皮下注射 · 3 天 1 次 · 体重区间剂量：200-400kg → 5ml/次；400-600kg → 10ml/次；600-900kg → 15ml/次；900kg 及以上 → 20ml/次",
          "氟尼辛葡甲胺注射液（福欣安） · 静脉注射 · 1 天 1 次，连用 3 天 · 体重区间剂量：200-400kg → 10ml/次；400-600kg → 20ml/次；600-900kg → 30ml/次；900kg 及以上 → 35ml/次",
          "直肠体温 · 检查 · 测量并记录牛只直肠体温 · 数字输入 · 14 天，1 天 1 次",
          "情况评估 · 检查 · 拍摄牛只正前方、正后方照片 · 图片视频 · 14 天，1 天 1 次",
        ],
  ...
  },
  {
    ...
    plans: [
      {
        id: "p1",
        rx: "处方 1 · 直肠体温 · 检查 + 情况评估 · 检查",
        desc: "产后检查；为期 14 天的产后检查任务，包含直肠体温数字记录和牛只正前方、正后方照片采集",
        drugs: [
          "直肠体温 · 检查 · 测量并记录牛只直肠体温 · 数字输入 · 14 天，1 天 1 次",
          "情况评估 · 检查 · 拍摄牛只正前方、正后方照片 · 图片视频 · 14 天，1 天 1 次",
        ],
